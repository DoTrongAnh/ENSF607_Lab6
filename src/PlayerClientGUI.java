...
public class PlayerClientGUI implements Constants{
	...
	public static void main(String[] args) {
		PlayerClientGUI window = new PlayerClientGUI("localhost", 8090);
		Thread t = new Thread() {
			public void run() {
				window.open();
				window.introduce();
	 ...
	 */
	public PlayerClientGUI(String serverName, int portNumber) {
		isTurn = false;
		gameStarted = false;
		//initialize();
	...
	public void play() {
		...
		try {
			while(true) {
				...
				if(read.equals("GAME OVER")) break;//End of game
			}
